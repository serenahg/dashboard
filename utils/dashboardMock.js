export const currentAccessStatus = [
  {
    name: "Sally Hameed",
    project: "B05 - B05",
    batch: "2 - Upholstery",
    dateTime: "21 - Apr-2023 8:10",
  },
  {
    name: "Reshma Gami",
    project: "B05 - B05",
    batch: "2 - Upholstery",
    dateTime: "22 - Apr-2023 8:10",
  },

  {
    name: "Sally Hameed",
    project: "B05 - B05",
    batch: "2 - Upholstery",
    dateTime: "21 - Apr-2023 8:10",
  },
  {
    name: "Sally Hameed",
    project: "B05 - B05",
    batch: "2 - Upholstery",
    dateTime: "21 - Apr-2023 8:10",
  },
  {
    name: "Sally Hameed",
    project: "B05 - B05",
    batch: "2 - Upholstery",
    dateTime: "21 - Apr-2023 8:10",
  },
];

export const recentActivities = [
  {
    project: "B05",
    "item Ref": "SS001",
    description: "Sofa 2 Seat",
    status: "IDA",
    "last Edited": "27-Apr-23",
    image: "/sofa.jpg",
  },
  {
    project: "B05",
    "item Ref": "SS451",
    description: "Sofa 3 Seat",
    status: "IDA",
    "last Edited": "27-Apr-23",
    image: "/sofa.jpg",
  },
  {
    project: "B02",
    "item Ref": "SS008",
    description: "Coffee table",
    status: "IDA",
    "last Edited": "27-Apr-23",
    image: "/sofa.jpg",
  },
  {
    project: "B04",
    "item Ref": "SS011",
    description: "Console Lamp",
    status: "IDA",
    "last Edited": "27-Apr-23",
    image: "/sofa.jpg",
  },
  {
    project: "B02",
    "item Ref": "SS008",
    description: "Coffee table",
    status: "IDA",
    "last Edited": "27-Apr-23",
    image: "/sofa.jpg",
  },
  {
    project: "B04",
    "item Ref": "SS011",
    description: "Console Lamp",
    status: "IDA",
    "last Edited": "27-Apr-23",
    image: "/sofa.jpg",
  },
  {
    project: "B02",
    "item Ref": "SS008",
    description: "Coffee table",
    status: "IDA",
    "last Edited": "27-Apr-23",
    image: "/sofa.jpg",
  },
  {
    project: "B04",
    "item Ref": "SS011",
    description: "Console Lamp",
    status: "IDA",
    "last Edited": "27-Apr-23",
    image: "/sofa.jpg",
  },
];

export const recentProjects = [
  { projects: "B05 - Farm", date: "27-Apr-23" },
  { projects: "MUD - Elevated Seating", date: "13-Mar-23" },
  { projects: "HID03 - Aljazirah", date: "12-Mar-23" },
  { projects: "C1150 - Golf Club", date: "01-Feb-23" },
  { projects: "C1150 - Golf Club", date: "01-Feb-23" },
];

export const orderStatus = [
  {
    status: "Order Processing",
    "n. of order": 10,
    "value in SAR": 90150,
  },
  {
    status: "Order Submitted Approval",
    "n. of order": 3,
    "value in SAR": 16456,
  },
  {
    status: "Order Approved",
    "n. of order": 4,
    "value in SAR": 1000,
  },

  {
    status: "Order Waiting Confirmation",
    "n. of order": 2,
    "value in SAR": 4250,
  },
];

export const supplierUpdates = [
  {
    supplier: "Ethnicraft",
    code: "ETHXX",
    "category of items": "console, tables",
    city: "London",
    country: "UK",
    "Account Type": "Account 30 Days",
  },
  {
    supplier: "John Lewis",
    code: "JLEWI",
    "category of items": "console, tables",
    city: "London",
    country: "UK",
    "Account Type": "Payment with order",
  },
  {
    supplier: "1srDibis",
    code: "FIRDI",
    "category of items": "Furniture, Artwork",
    city: "New York",
    country: "USA",
    "Account Type": "Payment with order",
  },
];

export const yourTenderReceveid = [
  {
    "Enquiry ID": 1024,
    supplier: "Sofa & Chair Co.",
    "Item Ref": "CL001",
    description: "Armchair",
    qty: 2.0,
    Uom: "EA",
    currency: "GBP",
    "Unit Price": "£2100.00",
    "Unit ": "£900.00",
  },
  {
    "Enquiry ID": 1024,
    supplier: "Sofa & Chair Co.",
    "Item Ref": "SS001",
    description: "Sofa 2 seats",
    qty: 4.0,
    Uom: "EA",
    currency: "GBP",
    "Unit Price": "£4100.00",
    "Unit ": "£900.00",
  },
  {
    "Enquiry ID": 1011,
    supplier: "Ethnicraft",
    "Item Ref": "TO001",
    description: "Coffee Table",
    qty: 2.0,
    Uom: "EA",
    currency: "GBP",
    "Unit Price": "£1500.00",
    "Unit ": "£900.00",
  },
  {
    "Enquiry ID": 1027,
    supplier: "Lumens",
    "Item Ref": "LT002",
    description: "Console Lamp",
    qty: 2.0,
    Uom: "EA",
    currency: "EU",
    "Unit Price": "£900.00",
    "Unit ": "£900.00",
  },
];

export const allProjects = [
  {
    projects: "B05 - Farm",
    "project number": "B05",
    "project name": "Farm",
    "project manager": "Marcelo Ortega",
    "project status": "Active",
    "project type": "LOC",
    "start date": "01-apr-2023",
    "last access date": "27-apr-23",
    date: "27-Apr-23",
    data: [
      {
        quantity: 0,
        name: "item sent to Site",
      },
      {
        quantity: 50,
        name: "Item Warehouse - Jeddah",
      },
      {
        quantity: 100,
        name: "Item Warehouse - Riyadh",
      },
      {
        quantity: 150,
        name: "Item Cleared Customs",
      },
      {
        quantity: 20,
        name: "Item in Customs",
      },
      {
        quantity: 150,
        name: "Item Shipped",
      },
      {
        quantity: 250,
        name: "Item on Shipment",
      },
      {
        quantity: 205,
        name: "Item Finished",
      },
      {
        quantity: 10,
        name: "Item Delivered",
      },
      {
        quantity: 410,
        name: "Item Ordered",
      },
      {
        quantity: 30,
        name: "Item on Order",
      },
      {
        quantity: 5,
        name: "Item Rejected",
      },
      {
        quantity: 30,
        name: "Item Approved",
      },
      {
        quantity: 10,
        name: "Item Submitted Approval",
      },
      {
        quantity: 300,
        name: "Item Detailing",
      },
    ],
  },
  {
    projects: "MUD - Elevated Seating",
    "project number": "MUD",
    "project name": "Elevating Seating",
    "project manager": "Marcelo Ortega",
    "project status": "Active",
    "project type": "LOC",
    "start date": "20-Oct-2022",
    "last access date": "13-Mar-23",
    date: "13-Mar-23",
    data: [
      {
        quantity: 10,
        name: "item sent to Site",
      },
      {
        quantity: 100,
        name: "Item Warehouse - Jeddah",
      },
      {
        quantity: 10,
        name: "Item Warehouse - Riyadh",
      },
      {
        quantity: 200,
        name: "Item Cleared Customs",
      },
      {
        quantity: 250,
        name: "Item in Customs",
      },
      {
        quantity: 100,
        name: "Item Shipped",
      },
      {
        quantity: 300,
        name: "Item on Shipment",
      },
      {
        quantity: 25,
        name: "Item Finished",
      },
      {
        quantity: 100,
        name: "Item Delivered",
      },
      {
        quantity: 40,
        name: "Item Ordered",
      },
      {
        quantity: 430,
        name: "Item on Order",
      },
      {
        quantity: 50,
        name: "Item Rejected",
      },
      {
        quantity: 20,
        name: "Item Approved",
      },
      {
        quantity: 40,
        name: "Item Submitted Approval",
      },
      {
        quantity: 250,
        name: "Item Detailing",
      },
    ],
  },
  {
    projects: "HID03 - Aljazirah",
    "project number": "HID03",
    "project name": "Aljazirah",
    "project manager": "Marcelo Ortega",
    "project status": "Active",
    "project type": "LOC",
    "start date": "01-jan-2023",
    "last access date": "12-mar-23",
    date: "12-Mar-23",
    data: [
      {
        quantity: 60,
        name: "item sent to Site",
      },
      {
        quantity: 50,
        name: "Item Warehouse - Jeddah",
      },
      {
        quantity: 10,
        name: "Item Warehouse - Riyadh",
      },
      {
        quantity: 100,
        name: "Item Cleared Customs",
      },
      {
        quantity: 20,
        name: "Item in Customs",
      },
      {
        quantity: 100,
        name: "Item Shipped",
      },
      {
        quantity: 20,
        name: "Item on Shipment",
      },
      {
        quantity: 25,
        name: "Item Finished",
      },
      {
        quantity: 10,
        name: "Item Delivered",
      },
      {
        quantity: 40,
        name: "Item Ordered",
      },
      {
        quantity: 300,
        name: "Item on Order",
      },
      {
        quantity: 50,
        name: "Item Rejected",
      },
      {
        quantity: 300,
        name: "Item Approved",
      },
      {
        quantity: 105,
        name: "Item Submitted Approval",
      },
      {
        quantity: 300,
        name: "Item Detailing",
      },
    ],
  },
  {
    projects: "C1150 - Golf Club",
    "project number": "C1150",
    "project name": "Golf Club",
    "project manager": "Marcelo Ortega",
    "project status": "closed",
    "project type": "LOC",
    "start date": "05-may-2023",
    "last access date": "12-mar-23",
    date: "12-Mar-23",
    data: [
      {
        quantity: 30,
        name: "item sent to Site",
      },
      {
        quantity: 100,
        name: "Item Warehouse - Jeddah",
      },
      {
        quantity: 100,
        name: "Item Warehouse - Riyadh",
      },
      {
        quantity: 150,
        name: "Item Cleared Customs",
      },
      {
        quantity: 200,
        name: "Item in Customs",
      },
      {
        quantity: 150,
        name: "Item Shipped",
      },
      {
        quantity: 250,
        name: "Item on Shipment",
      },
      {
        quantity: 105,
        name: "Item Finished",
      },
      {
        quantity: 104,
        name: "Item Delivered",
      },
      {
        quantity: 110,
        name: "Item Ordered",
      },
      {
        quantity: 30,
        name: "Item on Order",
      },
      {
        quantity: 50,
        name: "Item Rejected",
      },
      {
        quantity: 150,
        name: "Item Approved",
      },
      {
        quantity: 100,
        name: "Item Submitted Approval",
      },
      {
        quantity: 30,
        name: "Item Detailing",
      },
    ],
  },
  {
    projects: "B07 - Hill House",
    "project number": "B07",
    "project name": "Hill House",
    "project manager": "Marcelo Ortega",
    "project status": "Active",
    "project type": "LOC",
    "start date": "01-may-2023",
    "last access date": "12-jun-23",
    date: "14-Jun-23",
    data: [
      {
        quantity: 100,
        name: "item sent to Site",
      },
      {
        quantity: 80,
        name: "Item Warehouse - Jeddah",
      },
      {
        quantity: 190,
        name: "Item Warehouse - Riyadh",
      },
      {
        quantity: 150,
        name: "Item Cleared Customs",
      },
      {
        quantity: 205,
        name: "Item in Customs",
      },
      {
        quantity: 150,
        name: "Item Shipped",
      },
      {
        quantity: 250,
        name: "Item on Shipment",
      },
      {
        quantity: 205,
        name: "Item Finished",
      },
      {
        quantity: 10,
        name: "Item Delivered",
      },
      {
        quantity: 410,
        name: "Item Ordered",
      },
      {
        quantity: 30,
        name: "Item on Order",
      },
      {
        quantity: 50,
        name: "Item Rejected",
      },
      {
        quantity: 230,
        name: "Item Approved",
      },
      {
        quantity: 120,
        name: "Item Submitted Approval",
      },
      {
        quantity: 100,
        name: "Item Detailing",
      },
    ],
  },
  {
    projects: "MOR - Hall Pool",
    "project number": "MOR",
    "project name": "Hall Pool",
    "project manager": "Marcelo Ortega",
    "project status": "Active",
    "project type": "LOC",
    "start date": "04-feb-2023",
    "last access date": "13-Feb-23",
    date: "13-Feb-23",
    data: [
      {
        quantity: 20,
        name: "item sent to Site",
      },
      {
        quantity: 103,
        name: "Item Warehouse - Jeddah",
      },
      {
        quantity: 100,
        name: "Item Warehouse - Riyadh",
      },
      {
        quantity: 150,
        name: "Item Cleared Customs",
      },
      {
        quantity: 205,
        name: "Item in Customs",
      },
      {
        quantity: 150,
        name: "Item Shipped",
      },
      {
        quantity: 250,
        name: "Item on Shipment",
      },
      {
        quantity: 205,
        name: "Item Finished",
      },
      {
        quantity: 10,
        name: "Item Delivered",
      },
      {
        quantity: 210,
        name: "Item Ordered",
      },
      {
        quantity: 30,
        name: "Item on Order",
      },
      {
        quantity: 105,
        name: "Item Rejected",
      },
      {
        quantity: 70,
        name: "Item Approved",
      },
      {
        quantity: 103,
        name: "Item Submitted Approval",
      },
      {
        quantity: 100,
        name: "Item Detailing",
      },
    ],
  },
  {
    projects: "HID06 - Royal Garden",
    "project number": "HID06",
    "project name": "Royal Garden",
    "project manager": "Marcelo Ortega",
    "project status": "Active",
    "project type": "LOC",
    "start date": "04-nov-2023",
    "last access date": "01-Mar-23",
    date: "01-Mar-23",
    data: [
      {
        quantity: 5,
        name: "item sent to Site",
      },
      {
        quantity: 50,
        name: "Item Warehouse - Jeddah",
      },
      {
        quantity: 30,
        name: "Item Warehouse - Riyadh",
      },
      {
        quantity: 80,
        name: "Item Cleared Customs",
      },
      {
        quantity: 20,
        name: "Item in Customs",
      },
      {
        quantity: 15,
        name: "Item Shipped",
      },
      {
        quantity: 170,
        name: "Item on Shipment",
      },
      {
        quantity: 205,
        name: "Item Finished",
      },
      {
        quantity: 10,
        name: "Item Delivered",
      },
      {
        quantity: 320,
        name: "Item Ordered",
      },
      {
        quantity: 30,
        name: "Item on Order",
      },
      {
        quantity: 3,
        name: "Item Rejected",
      },
      {
        quantity: 30,
        name: "Item Approved",
      },
      {
        quantity: 80,
        name: "Item Submitted Approval",
      },
      {
        quantity: 100,
        name: "Item Detailing",
      },
    ],
  },
  {
    projects: "C1180 - Golf Club",
    "project number": "C1180",
    "project name": "Golf Club",
    "project manager": "Bernardo Martos",
    "project status": "Active",
    "project type": "LOC",
    "start date": "04-sep-2023",
    "last access date": "01-Feb-23",
    date: "01-Feb-23",
    data: [
      {
        quantity: 0,
        name: "item sent to Site",
      },
      {
        quantity: 50,
        name: "Item Warehouse - Jeddah",
      },
      {
        quantity: 100,
        name: "Item Warehouse - Riyadh",
      },
      {
        quantity: 150,
        name: "Item Cleared Customs",
      },
      {
        quantity: 20,
        name: "Item in Customs",
      },
      {
        quantity: 150,
        name: "Item Shipped",
      },
      {
        quantity: 250,
        name: "Item on Shipment",
      },
      {
        quantity: 205,
        name: "Item Finished",
      },
      {
        quantity: 10,
        name: "Item Delivered",
      },
      {
        quantity: 410,
        name: "Item Ordered",
      },
      {
        quantity: 30,
        name: "Item on Order",
      },
      {
        quantity: 5,
        name: "Item Rejected",
      },
      {
        quantity: 30,
        name: "Item Approved",
      },
      {
        quantity: 10,
        name: "Item Submitted Approval",
      },
      {
        quantity: 300,
        name: "Item Detailing",
      },
    ],
  },
];

export const data = [
  {
    quantity: 0,
    name: "item sent to Site",
  },
  {
    quantity: 50,
    name: "Item Warehouse - Jeddah",
  },
  {
    quantity: 100,
    name: "Item Warehouse - Riyadh",
  },
  {
    quantity: 150,
    name: "Item Cleared Customs",
  },
  {
    quantity: 20,
    name: "Item in Customs",
  },
  {
    quantity: 150,
    name: "Item Shipped",
  },
  {
    quantity: 250,
    name: "Item on Shipment",
  },
  {
    quantity: 205,
    name: "Item Finished",
  },
  {
    quantity: 10,
    name: "Item Delivered",
  },
  {
    quantity: 410,
    name: "Item Ordered",
  },
  {
    quantity: 30,
    name: "Item on Order",
  },
  {
    quantity: 5,
    name: "Item Rejected",
  },
  {
    quantity: 30,
    name: "Item Approved",
  },
  {
    quantity: 10,
    name: "Item Submitted Approval",
  },
  {
    quantity: 300,
    name: "Item Detailing",
  },
];

export const latestPaymentStatus = [
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
  {
    "Pay Id": 1024,
    project: "B05",
    purch: "CL001",
    order: 1050,
    suppl: "ETHXX",
    "Order Description": "PMV FOrniture",
    "Issue Date": "24-Apr-2023",
    curr: "GBP",
    value: "£2100.00",
    "Payment Value": "£2250.00",
    "Payment Terms": "100% with Order",
    "Requiered Date": "28-1pr-2023",
  },
];
export const recentOrderUpdates = [
  {
    order: 1024,
    project: "B05",
    purch: "CL001",
    supplier: "ETHXX",
    "Order Description": "PMV FOrniture",
    status: "OI",
    curr: "GBP",
    value: "£2100.00",
    by: "Ghaida Alaswad",
  },
  {
    order: 1024,
    project: "B05",
    purch: "CL001",
    supplier: "ETHXX",
    "Order Description": "PMV FOrniture",
    status: "OI",
    curr: "GBP",
    value: "£2100.00",
    by: "Ghaida Alaswad",
  },
  {
    order: 1028,
    project: "B09",
    purch: "CL003",
    supplier: "FIRDI",
    "Order Description": "PSV Upholstery",
    status: "OI",
    curr: "GBP",
    value: "£2100.00",
    by: "Ghaida Alaswad",
  },
  {
    order: 1300,
    project: "B09",
    purch: "HIDEU",
    supplier: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    curr: "EUR",
    value: "£3100.00",
    by: "Maria Salazar",
  },
  {
    order: 1024,
    project: "B05",
    purch: "CL001",
    supplier: "ETHXX",
    "Order Description": "PMV FOrniture",
    status: "OI",
    curr: "GBP",
    value: "£2100.00",
    by: "Ghaida Alaswad",
  },
  {
    order: 1024,
    project: "B05",
    purch: "CL001",
    supplier: "ETHXX",
    "Order Description": "PMV FOrniture",
    status: "OI",
    curr: "GBP",
    value: "£2100.00",
    by: "Ghaida Alaswad",
  },
  {
    order: 1028,
    project: "B09",
    purch: "CL003",
    supplier: "FIRDI",
    "Order Description": "PSV Upholstery",
    status: "OI",
    curr: "GBP",
    value: "£2100.00",
    by: "Ghaida Alaswad",
  },
  {
    order: 1300,
    project: "B09",
    purch: "HIDEU",
    supplier: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    curr: "EUR",
    value: "£3100.00",
    by: "Maria Salazar",
  },
];

export const latestExpeditingNotes = [
  {
    order: 1300,
    proj: "B09",
    purch: "HIDEU",
    suppl: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    "Date & Time": "1-Apr-2023",
    "Latest Note":
      "Foto received for the last two pieces of forniture, Jennifer advised packing to be completed",
    "Action Date": "27-May-2023",
  },
  {
    order: 1300,
    proj: "B09",
    purch: "HIDEU",
    suppl: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    "Date & Time": "1-Apr-2023",
    "Latest Note":
      "Foto received for the last two pieces of forniture, Jennifer advised packing to be completed",
    "Action Date": "27-May-2023",
  },
  {
    order: 1300,
    proj: "B09",
    purch: "HIDEU",
    suppl: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    "Date & Time": "1-Apr-2023",
    "Latest Note":
      "Foto received for the last two pieces of forniture, Jennifer advised packing to be completed",
    "Action Date": "27-May-2023",
  },
  {
    order: 1300,
    proj: "B09",
    purch: "HIDEU",
    suppl: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    "Date & Time": "1-Apr-2023",
    "Latest Note":
      "Foto received for the last two pieces of forniture, Jennifer advised packing to be completed",
    "Action Date": "27-May-2023",
  },
  {
    order: 1300,
    proj: "B09",
    purch: "HIDEU",
    suppl: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    "Date & Time": "1-Apr-2023",
    "Latest Note":
      "Foto received for the last two pieces of forniture, Jennifer advised packing to be completed, Jennifer advised packing to be completed",
    "Action Date": "27-May-2023",
  },
  {
    order: 1300,
    proj: "B09",
    purch: "HIDEU",
    suppl: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    "Date & Time": "1-Apr-2023",
    "Latest Note": "Foto received for the last two pieces of forniture, ",
    "Action Date": "27-May-2023",
  },
  {
    order: 1300,
    proj: "B09",
    purch: "HIDEU",
    suppl: "ETHXX",
    "Order Description": "Glassware",
    status: "OIG",
    "Date & Time": "1-Apr-2023",
    "Latest Note": "Foto received for the last two pieces of",
    "Action Date": "27-May-2023",
  },
];

export const userInfo2 = {
  name: "Marco Olivieri",
  dpt: "Purchasing",
  dashboard: [
    {
      title: "Number of Items",
      content: allProjects,
      kind: "chart",
    },
    {
      title: "Order Status",
      content: orderStatus,
      kind: "table",
    },
    {
      title: "Recent Order Updates",
      content: recentOrderUpdates,
      kind: "table",
    },
    {
      title: "Latest Payment Status",
      content: latestPaymentStatus,
      kind: "table",
    },
    {
      title: "LatestExpeditingNotes",
      content: latestExpeditingNotes,
      kind: "table",
      linkables: [{ purch: "/dashboard/projects/project-list" }],
    },

    {
      title: "Your Tender Received",
      content: yourTenderReceveid,
      kind: "table",
    },
  ],
};
export const userInfo = {
  name: "Serena Olivieri",
  dpt: "FF&E",
  dashboard: [
    {
      title: "Number of Items",
      content: allProjects,
      kind: "chart",
    },
    {
      title: "Current Access Status",
      content: currentAccessStatus,
      kind: "table",
    },
    {
      title: "Recent Projects",
      content: recentProjects,
      kind: "table",
      linkables: [{ projects: "/dashboard/projects/project-list" }],
    },
    {
      title: "Recent Activities",
      content: recentActivities,
      kind: "table",
      linkables: [{ project: "/dashboard/projects/project-list" }],
    },
    {
      title: "Recent Activities",
      content: recentActivities,
      kind: "table",
    },

    {
      title: "Your Tender Received",
      content: yourTenderReceveid,
      kind: "table",
    },
  ],
};
